...
int conf_parse_stream(conf_reader reader, void *stream, conf_handler handler, void *_pcfg) {
	...
	while (reader(line, CONF_MAX_LINE, stream) != NULL) {
		lineno++;
		...
		if (*start == ';' || *start == '#') {
			...
			if (!handler(_pcfg, section, prev_name, start) && !error)
				error = lineno;
		}
		else if (*start == '[') {
			/* A "[section]" line */
			...
			if (*end == '=' || *end == ':') {
				*end = '\0';
				...
				if (!handler(_pcfg, section, name, value) && !error)
					error = lineno;
			}
			else if (!error) {
...
int conf_parse_file(FILE *file, conf_handler handler, void *_pcfg) {
	return conf_parse_stream((conf_reader)fgets, file, handler, _pcfg);
}
...
int conf_parse(const char *filename, conf_handler handler, void *_pcfg) {
	...
	if (!file)
		return -1;
	error = conf_parse_file(file, handler, _pcfg);
	fclose(file);
	return error;
}
